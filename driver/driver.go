...
var (
	...
	taskConfigSpec = hclspec.NewObject(map[string]*hclspec.Spec{
		...
		"OCIImage":    hclspec.NewAttr("OCIImage", "string", false),
		...
		"OCIAuth": hclspec.NewBlock("OCIAuth", false, hclspec.NewObject(map[string]*hclspec.Spec{
...
type TaskConfig struct {
	...
	OCIImage    string   `codec:"OCIImage"`
	Disks       []string `codec:"Disks"`
	Network     string   `codec:"Network"`
	Nic         Nic      `codec:"Nic"`
	OCIAuth     OCIAuth  `codec:"OCIAuth"`
...
// validateTaskConfig validates the task configuration with simplified logic
func (d *Driver) validateTaskConfig(config TaskConfig) error {
	// Check that either BootDisk or OCIImage is specified, but not both
	hasBootDisk := config.BootDisk != ""
	hasOCIImage := config.OCIImage != ""
	
	if hasBootDisk && hasOCIImage {
		return fmt.Errorf("cannot specify both BootDisk and OCIImage - choose one")
	}
	
	if !hasBootDisk && !hasOCIImage {
		return fmt.Errorf("must specify either BootDisk or OCIImage")
	}

	// Simple OCI image validation
	if hasOCIImage && strings.TrimSpace(config.OCIImage) == "" {
		return fmt.Errorf("OCIImage cannot be empty")
	}

	// Validate network configuration
	if config.Network != "" && config.Nic.Ip != "" {
		return fmt.Errorf("cannot specify both Network (CNI) and Nic.Ip (static) - choose one")
	}
