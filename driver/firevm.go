...
func (d *Driver) initializeContainer(ctx context.Context, cfg *drivers.TaskConfig, taskConfig TaskConfig) (*vminfo, error) {
	// Handle OCI image conversion if specified
	if len(taskConfig.OCIImage) > 0 {
		if err := d.handleOCIImage(ctx, taskConfig, cfg); err != nil {
			return nil, fmt.Errorf("failed to handle OCI image: %v", err)
		}
	}
...
// handleOCIImage manages OCI image pulling and rootfs creation using simplified approach
func (d *Driver) handleOCIImage(ctx context.Context, taskConfig TaskConfig, cfg *drivers.TaskConfig) error {
	d.logger.Info("handling OCI image", "image", taskConfig.OCIImage)

	// Create simplified OCI manager
	workDir := filepath.Join(cfg.AllocDir, cfg.Name)
	ociManager, err := NewSimpleOCIManager(d.logger, workDir)
	if err != nil {
		return fmt.Errorf("failed to create OCI manager: %w", err)
	}
	defer ociManager.Close()

	// Ensure the allocation directory exists
	if err := os.MkdirAll(workDir, 0755); err != nil {
		return fmt.Errorf("failed to create allocation directory: %w", err)
	}
	...
	if err := ociManager.PullAndCreateRootfs(ctx, taskConfig.OCIImage, rootfsPath, auth); err != nil {
		return fmt.Errorf("failed to create rootfs from OCI image: %w", err)
	}

	d.logger.Info("successfully created rootfs from OCI image", "image", taskConfig.OCIImage, "rootfs", rootfsPath)
	return nil
}
